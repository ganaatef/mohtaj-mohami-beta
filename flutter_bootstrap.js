...
const startupTimeout = setTimeout(() => {
  message.textContent = 'التحميل أطول من المعتاد. تحقق من الإنترنت ثم أعد المحاولة.';
...
async function clearLegacyWorkers() {
  ...
  try {
    ...
    const appRegistrations = registrations.filter((registration) =>
      new URL(registration.scope).pathname.startsWith('/mohtaj-mohami-beta/'));
    await Promise.all(appRegistrations.map((registration) => registration.unregister()));
  } catch (_) {
    // A restricted browser may deny service-worker access; app loading can continue.
  }
}

function scheduleLegacyWorkerCleanup() {
  const cleanup = () => clearLegacyWorkers();
...
_flutter.loader.load({
  onEntrypointLoaded: async (engineInitializer) => {
    message.textContent = 'جاري تجهيز الواجهة...';
    const appRunner = await engineInitializer.initializeEngine();
    await appRunner.runApp();
    clearTimeout(startupTimeout);
    loader.classList.add('hidden');
    setTimeout(() => loader.remove(), 220);
    scheduleLegacyWorkerCleanup();
  },
}).catch((error) => {
  console.error('Application startup failed', error);
  clearTimeout(startupTimeout);
  message.textContent = 'تعذر فتح المنصة على هذا الجهاز. اضغط إعادة المحاولة.';
  retry.style.display = 'block';
});
