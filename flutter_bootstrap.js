...
const startupTimeout = setTimeout(() => {
  message.textContent = 'الاتصال بطيء أو المتصفح قديم. تحقق من الإنترنت ثم أعد المحاولة.';
...
clearLegacyWorkers()
  .then(() => _flutter.loader.load({
    onEntrypointLoaded: async (engineInitializer) => {
      message.textContent = 'يتم تجهيز الواجهة...';
  ...
  .catch((error) => {
    ...
    clearTimeout(startupTimeout);
    message.textContent = 'تعذر فتح التطبيق على هذا الجهاز. اضغط إعادة المحاولة.';
    retry.style.display = 'block';
  });
